      ...
      <div style={{
        ...
        {loading && (
          <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16 }}>
            <div style={{ width: 28, height: 28, border: `2px solid ${t.borderLight}`, borderTopColor: "#4C9EFF", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
        ...
        {error && (
          <div style={{ padding: 24 }}>
            ...
            <button onClick={() => { setMode(null); setError(null); }} style={{ marginTop: 12, fontSize: 12, color: "#4C9EFF", background: "none", border: "none", cursor: "pointer" }}>{"\u2190"} Try again</button>
          </div>
        )}
        ...
        {result && !loading && (
          <div style={{ flex: 1, overflow: "auto", padding: 24 }}>
            <button onClick={() => { setMode(null); setResult(null); }} style={{ fontSize: 11, color: "#4C9EFF", background: "none", border: "none", cursor: "pointer", marginBottom: 16 }}>{"\u2190"} Back</button>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {data.stalls?.length > 0 ? data.stalls.map((s, i) => (
        ...
        <div style={{ fontSize: 12, color: "#3DDC84" }}>No stalls detected. Keep pushing!</div>
      )}
    </div>
  );
}
        ...
        <div key={i} style={{ background: t.surface, border: `1px solid ${t.border}`, borderRadius: 12, padding: 16 }}>
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            {s.alternatives?.map((alt, j) => (
              <span key={j} style={{ fontSize: 11, padding: "4px 10px", borderRadius: 8, background: "rgba(76,158,255,0.08)", color: "#4C9EFF", border: "1px solid rgba(76,158,255,0.15)" }}>{alt}</span>
            ))}
          </div>
        </div>
      ))}
