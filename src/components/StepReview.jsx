  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 24 }}>
        {[{ l: "Training Days", v: `${trainDays}/wk`, c: "#3DDC84" }, { l: "Exercises", v: totalEx, c: "#4C9EFF" }, { l: "Weekly Sets", v: totalSets, c: "#FBBF24" }, { l: "Mesocycle", v: `${plan.weeks} wks`, c: "#A78BFA" }].map(c => <div key={c.l} style={{ background: t.surface, borderRadius: 12, padding: 20, textAlign: "center" }}><div style={{ fontSize: 24, fontFamily: "inherit", fontWeight: 700, color: c.c }}>{c.v}</div><div style={{ fontSize: 11, color: t.textDim, marginTop: 4 }}>{c.l}</div></div>)}
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 8, marginBottom: 24 }}>
        {wt.map((day, i) => <div key={i} style={{ background: day.isRest ? t.surface2 : t.surface, borderRadius: 12, padding: 16, minHeight: 100, opacity: day.isRest ? 0.5 : 1 }}><div style={{ fontSize: 9, fontFamily: "inherit", color: t.textFaint, marginBottom: 4 }}>{DAY_NAMES[(i + 1) % 7]}</div><div style={{ fontSize: 13, fontWeight: 700, color: t.text, marginBottom: 6 }}>{day.label}</div>{!day.isRest && day.exercises.slice(0, 3).map((e, ei) => <div key={ei} style={{ fontSize: 9, color: t.textDim, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{EXERCISES[e.id]?.name}</div>)}{day.isRest && <div style={{ fontSize: 22, marginTop: 8 }}>😴</div>}</div>)}
      </div>
      <div style={{ background: t.surface, borderRadius: 12, padding: 24 }}>
        <div style={{ fontSize: 13, fontWeight: 600, color: t.textMuted, marginBottom: 14 }}>Predicted Weekly Coverage</div>
