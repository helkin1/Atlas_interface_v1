...
function StepAboutYou({ profile, onChange, t }) {
  ...
  const setHeight = (val) => {
    ...
    if (isNaN(num)) { onChange({ ...profile, heightCm: null }); return; }
    onChange({ ...profile, heightCm: isMetric ? num : Math.round(num * 2.54) });
  };
  const setWeight = (val) => {
    const num = parseFloat(val);
    if (isNaN(num)) { onChange({ ...profile, weightKg: null }); return; }
    onChange({ ...profile, weightKg: isMetric ? num : Math.round(num / 2.205) });
  };
      ...
      <div>
        ...
        <div style={{ display: "flex", gap: 4, background: t.surface2, borderRadius: 8, padding: 2, width: "fit-content" }}>
          {["imperial", "metric"].map(u => (
            ...
            }}>{u === "imperial" ? "Imperial (lbs/in)" : "Metric (kg/cm)"}</button>
          ))}
        </div>
      </div>
